...
static void update_current_map(window_controller_t *manager)
{
    ...
    current = (current + NUMBER_GROUNDS - 1) % NUMBER_GROUNDS;
    data->settings.block_img = GROUNDS[current];
    sfClock_restart(manager->clock);
}

int switch_ground_prev(object_entity_t *obj,
        ...
        window_controller_t *manager)
{
    ...
    sfText_setString(obj->text, "<- ground prev");
