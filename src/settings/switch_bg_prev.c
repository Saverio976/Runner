...
static void update_current_map(window_controller_t *manager)
{
    ...
    current = (current + NUMBER_BACKGS - 1) % NUMBER_BACKGS;
    data->settings.background_img = BACKGROUNDS[current];
    sfClock_restart(manager->clock);
}

int switch_bg_prev(object_entity_t *obj,
        ...
        window_controller_t *manager)
{
    ...
    sfText_setString(obj->text, "<- background prev");
